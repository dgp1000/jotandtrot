...
3a. **Continue with Google** — ✅ built 10 Aug 2026, web/PWA only (Google forbids OAuth in embedded webviews; the iOS app keeps Apple + email, which also satisfies Apple's rule 4.8). GCP project "My Project 67658": consent screen "Jot & Trot" + web OAuth client, published to production (basic scopes, no verification needed). Supabase Google provider enabled. Same-email accounts link automatically.
